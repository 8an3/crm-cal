...
import { UserPlus, Gauge, CalendarPlus, ChevronsLeft, Banknote, Phone, CalendarCheck, ChevronsRight, Truck, ArrowLeft, ArrowDownToDot, ArrowRight, ClipboardCheck, Clipboard, X, ChevronRightIcon, ChevronLeftIcon, Copy, CalendarIcon, } from "lucide-react";
import EventInfoModal from "../components/EventInfoModal";
import AddCustomerModal from "../components/addCustomerModal";
import { Button, buttonVariants, Tooltip, TooltipContent, TooltipProvider, TooltipTrigger, Popover, PopoverTrigger, PopoverContent, Input, Separator, } from "../components/ui";
import { Select, SelectContent, SelectGroup, SelectItem, SelectLabel, SelectTrigger, SelectValue, } from "../components/ui/select";
import { useScreenSize, ViewNamesGroup, CustomToolbar, mobileToolbar, noToolbar, colors, IEventInfo, EventFormData, DatePickerEventFormData, initialEventFormState, initialDatePickerEventFormData, IProps, ViewToolbar, HoverEvent, AgendaTime, AgendaDate, AppointmentInfo, UpdateCompeltedAppt, } from "../components/shared";
...
export default function Sales() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await axios.get(`${VITE_API_URL}/sales/calendar/apts/get`,); // {  withCredentials: true,  }
        console.log("hit axios", response);
        const data = response.data.data;
        setUser(data.user);
        setSalesData(data.salesData);
        const formattedData = data.salesData?.map((event) => ({ ...event, start: new Date(event.start), end: new Date(event.end) }));
  ...
  const formattedData = salesData?.map((event) => ({ ...event, start: new Date(event.start), end: new Date(event.end) }));
  ...
  const onEventResize: withDragAndDropProps["onEventResize"] = (data) => {
    ...
    try {
      const updateAPt = async () => {
        const response = await fetch(`${VITE_API_URL}/sales/calendar/apt`, {
          method: "POST",
          headers: {
  ...
  async function FetchData(id) {
    const url = `${VITE_API_URL}/sales/calendar/apt/get/${id}`;
  ...
  const resourceTitle = (resource) => {
    return (
      <div className=" justify-center items-center flex my-3 ">
        ...
        <p className="text-foreground text-center text-2xl my-auto ml-3">
              ...
              <div className=" mt-3 grid grid-cols-1  justify-center mx-auto">
                ...
                <div>
                  <Select
                    ...
                    onValueChange={(value) => {
                      handleResourceChange(Number(value));
                      if (value === "0") {
                        setShowResources(true);
                      } else {
                        setShowResources(false);
                ...
                <Button
                  variant="outline"
                  className=" px-4 mx-auto mt-3 text-foreground cursor-pointer hover:text-primary justify-center items-center border-border hover:border-primary  bg-transparent hover:bg-transparent w-[180px] "
                  onClick={() => setAddCustomerModal(true)}
                >
                  <>
                    <UserPlus size={20} strokeWidth={1.5} />
                    <p className="ml-2">Add Customer</p>
                  </>
                </Button>
                <a
                  className="  mx-auto "
                  href={VITE_APP_URL + `/portal/sales/dashboard`}
                >
                  <Button
                    variant="outline"
                ...
                <Button
                  variant="outline"
                  className=" px-4 mt-3 mx-auto text-foreground cursor-pointer hover:text-primary justify-center items-center   border-border hover:border-primary bg-transparent hover:bg-transparent  w-[180px] "
                  onClick={() => setAddApptModal(true)}
                >
                  <>
                    <CalendarPlus size={20} strokeWidth={1.5} />
                    <p className="ml-2">Add Appointment</p>
                  </>
                </Button>
              </div>
            </div>
